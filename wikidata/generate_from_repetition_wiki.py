...
def main(
    *,
    ...
    global encoded_prompts, original_prediction
    ...
    prompt_category = np.array(prompt_category)
    print("shape checks: ", new_embed_all.shape, original_embed_all.shape, prompt_type.shape, prompt_category.shape)    

    # balance two classes
    cls_one = prompt_type.sum()
    cls_zero = len(prompt_type) - prompt_type.sum()
    if cls_one != cls_zero:
        if cls_one > cls_zero:
            small_cls, big_cls = 0, 1
        else:
            small_cls, big_cls = 1, 0
        small_cls_idx = torch.argwhere(prompt_type==small_cls)
        big_cls_dix = torch.argwhere(prompt_type==big_cls)

        big_cls_select = torch.randperm(len(big_cls_dix))
        big_cls_idx = big_cls_select[:len(small_cls_idx), None]  

        cls_idx = torch.concatenate([small_cls_idx, big_cls_idx]).flatten()    

        new_embed_all = new_embed_all[cls_idx]
        original_embed_all = original_embed_all[cls_idx]
        prompt_type = prompt_type[cls_idx]
        prompt_category = prompt_category[cls_idx]
        # encoded_prompts = encoded_prompts[(cls_idx.flatten().tolist())]
    ...
    print("entropy shape: ", original_entropy.shape, new_entropy.shape)

    original_prediction = torch.argmax(original_probs, dim=-1)

    global img_output_dir

    for category in np.unique(prompt_category):
        catrgoey_idx = np.argwhere(prompt_category==category).flatten()
        print("category: ", category, len(catrgoey_idx))

        if output_dir is not None:
            img_output_dir = output_dir
        else: 
            img_output_dir = os.path.join(os.path.dirname(repetition_dir), "results", repetition_dir.split("/")[-1], category)
            print("image output dir:", img_output_dir)
            Path(img_output_dir).mkdir(parents=True, exist_ok=True)


        plot_entropy(original_entropy.numpy()[catrgoey_idx], new_entropy.numpy()[catrgoey_idx], prompt_type.numpy()[catrgoey_idx])

        mutual_information(original_probs[catrgoey_idx], new_probs[catrgoey_idx], prompt_type[catrgoey_idx])

        weighted_entropy(original_entropy[catrgoey_idx], new_entropy[catrgoey_idx], original_probs[catrgoey_idx], prompt_type[catrgoey_idx])

        # topk_lift(original_probs[catrgoey_idx], new_probs[catrgoey_idx], prompt_type[catrgoey_idx])
        
        # probs_norm(original_probs, new_probs, prompt_type)

        # kl_divergence(original_probs, new_probs, prompt_type)

    # img_output_dir = os.path.join(os.path.dirname(repetition_dir), "results", repetition_dir.split("/")[-1], "all")
    # print("image output dir:", img_output_dir)
    # Path(img_output_dir).mkdir(parents=True, exist_ok=True)

    # plot_entropy(original_entropy.numpy(), new_entropy.numpy(), prompt_type.numpy())
...
def mutual_information(x_probs, y_probs, prompt_type):
    ...
    print("--- MI classifier ---")
    simple_classification(HYX.to(DEVICE), prompt_type.to(DEVICE))
...
def plot_entropy(original_entropy, new_entropy, prompt_type):
    plt.figure()
    plt.hist(new_entropy[prompt_type].flatten(), bins=50, histtype="step", label="ALL(new low_e_high_a)")
    plt.hist(new_entropy[~prompt_type].flatten(), bins=50, histtype="step", label="ALL(new high_e_low_a)")
    ...
    plt.figure()
    plt.hist(new_entropy[prompt_type, 0], bins=50, histtype="step", label="ALL(new low_e_high_a)")
    plt.hist(new_entropy[~prompt_type, 0].flatten(), bins=50, histtype="step", label="ALL(new high_e_low_a)")
    plt.hist(original_entropy[prompt_type].flatten(), bins=30, histtype="step", label="original low_e_high_a", alpha=0.5)
    plt.hist(original_entropy[~prompt_type].flatten(), bins=30, histtype="step", label="original high_e_low_a", alpha=0.5)
    plt.xlabel("Entropy")
    plt.ylabel("Count")
    plt.legend()
    plt.savefig(f"{img_output_dir}/new_entropy_first.png", bbox_inches='tight')

    plt.figure(figsize=(12, 12))
    plt.subplot(2, 2, 1)
    plt.hist(new_entropy[prompt_type].min(axis=1)- original_entropy[prompt_type], bins=50, histtype="step", label="MIN(new low_e_high_a) - orig")
    plt.hist(new_entropy[~prompt_type].min(axis=1) - original_entropy[~prompt_type], bins=50, histtype="step", label="MIN(new high_e_low_a) - orig")
    plt.hist(original_entropy[prompt_type].flatten(), bins=30, histtype="step", label="original low_e_high_a", alpha=0.5)
    plt.hist(original_entropy[~prompt_type].flatten(), bins=30, histtype="step", label="original high_e_low_a", alpha=0.5)
    plt.xlabel("Entropy")
    plt.title("A")
    ...
    plt.subplot(2, 2, 2)
    plt.hist(new_entropy[prompt_type].min(axis=1)- original_entropy[prompt_type], bins=50, histtype="step", label="MIN(new low_e_high_a) - orig")
    plt.hist(new_entropy[~prompt_type].max(axis=1)- original_entropy[~prompt_type], bins=50, histtype="step", label="MAX(new high_e_low_a) - orig")
    plt.hist(original_entropy[prompt_type].flatten(), bins=30, histtype="step", label="original low_e_high_a", alpha=0.5)
    plt.hist(original_entropy[~prompt_type].flatten(), bins=30, histtype="step", label="original high_e_low_a", alpha=0.5)
    plt.xlabel("Entropy")
    plt.ylabel("Count")
    ...
    plt.subplot(2, 2, 3)
    plt.hist(new_entropy[prompt_type].max(axis=1) - original_entropy[prompt_type], bins=50, histtype="step", label="MAX(new low_e_high_a) - orig")
    plt.hist(new_entropy[~prompt_type].min(axis=1) - original_entropy[~prompt_type], bins=50, histtype="step", label="MIN(new high_e_low_a) - orig")
    plt.hist(original_entropy[prompt_type].flatten(), bins=30, histtype="step", label="original low_e_high_a", alpha=0.5)
    plt.hist(original_entropy[~prompt_type].flatten(), bins=30, histtype="step", label="original high_e_low_a", alpha=0.5)
    plt.xlabel("Entropy")
    ...
    # simple_classification(torch.from_numpy(new_entropy.min(axis=1)).to(DEVICE), torch.from_numpy(prompt_type).to(DEVICE))
...
def topk_lift(x_probs, y_probs, prompt_type):
    px = torch.topk(x_probs, 10, dim=1)
    top_k_idx = px.indices
    p_x = px.values
    p_x = torch.div(p_x, p_x.sum(dim=1, keepdim=True))

    p_y = torch.zeros_like(p_x)
    for i in range(top_k_idx.shape[0]):
        for j in range(top_k_idx.shape[1]):
            p_y[i, j] = y_probs[i, j, top_k_idx[i, j]]
    
    p_y_norm = torch.zeros_like(p_x)
    for i in range(top_k_idx.shape[0]):
        for j in range(top_k_idx.shape[1]):
            p_y_norm[i, j] = (y_probs[i, j, top_k_idx[i]]).sum()
    p_y = torch.div(p_y, p_y_norm)
    
    prob_lift = p_y - p_x
    weighted_prob_lift = (prob_lift).mean(axis=1)

    plt.figure()
    plt.hist(prob_lift[prompt_type].log().cpu().numpy().flatten(), bins=50, histtype="step", label="low_e_high_a")
    plt.hist(prob_lift[~prompt_type].log().cpu().numpy().flatten(), bins=50, histtype="step", label="high_e_low_a")
    plt.xlabel("Probability Lift")
    plt.ylabel("Count")
    plt.legend()
    plt.savefig(f"{img_output_dir}/probs_lift.png", bbox_inches='tight')
    
    
    print("--- probability lift classifier ---")
    simple_classification(weighted_prob_lift.to(DEVICE), prompt_type.to(DEVICE))
    return
...
def simple_classification(x_train, y_train):
    ...
    for i in range(2001):
        ...
        if i == 2000: 
            ...
            prediction = output.detach().view(-1) < 0
            accuracy_2 = torch.sum((prediction==y_train).bool()) / y_train.shape[0]
            print(f"\tEpoch {i}, Loss:{loss.item():.3f}, Acc:{accuracy.item():.2f}, Flip Acc:{accuracy_2.item():.2f}")
    print((prediction).sum(), (~prediction).sum(), model.bias)
